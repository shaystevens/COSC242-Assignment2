 ...
 *
 * PURPOSE - asgn2, the main file, uses a tree to perform a number of
 * tasks. By default, words are read from stdin and added to your tree
...
/**
 * Prints out the keys/words and how many times they were input into the
 * tree.
 *
 ...
 *
 * @param x - int, to determine if help message called due to -h option
 * given or because invalid option given.
 */
            ...
            switch (option) {
                case 'c':
                    if(NULL == (filename = fopen(optarg, "r"))){
                        fprintf(stderr,
                                "Can't open file '%s' using mode r.\n",
                                optarg);
                        return EXIT_FAILURE;
                    }
                    check_file = 1;
